...
namespace {
    ...
    pluginCheck(count($scheduler->addedTasks) === 5, 'registerSchedules() must register exactly five tasks');
    pluginCheck(
    ...
    pluginCheck(
        $scheduler->addedTasks[2] instanceof \APP\plugins\generic\chatwootIntegration\classes\v2\Task\FaqCacheSyncScheduledTask,
        'the third registered task must be the real FaqCacheSyncScheduledTask (KNO-011), not a stand-in'
    );
    pluginCheck(
        $scheduler->addedTasks[3] instanceof \APP\plugins\generic\chatwootIntegration\classes\v2\Task\DeliverQueuedSupportEventsTask,
        'the fourth registered task must be the real DeliverQueuedSupportEventsTask, not a stand-in'
    );
    pluginCheck(
        $scheduler->addedTasks[4] instanceof \APP\plugins\generic\chatwootIntegration\classes\v2\Task\ProcessLegacyRetryQueueScheduledTask,
        'the fifth registered task must be the real ProcessLegacyRetryQueueScheduledTask (EVT-018) — the legacy apiQueue\'s reliable scheduler-only consumer, replacing the removed per-page-render drain'
    );
