...
Last reconciled: 2026-09-04 (PR #231 merged — HAR-012 settings-semantics ambiguity closed).
...
3. While implementing the console, close the cross-cutting hardening items that directly affect the same surfaces. HAR-008 closed above; HAR-018's `skipBackendPages` placebo-setting item closed by PR #211, other HAR-018 items remain open; HAR-007 closed by PR #213 — also fixed a real, high-volume production bug found along the way: a nonexistent `Repo::context()` call was throwing 3,081 times/day in real dell traffic, now 0; HAR-006 closed by PR #215 — widget and bind now share one `resolveReviewerMasking()` decision instead of two independently-maintained ones; HAR-001 partially closed by PRs #217/#229 — `ChatwootApiService` now fails closed instead of silently guessing account 1 when account resolution fails (live-verified against the real Chatwoot API on dell against both a bad token and the real production account), and caches the resolved account per (baseUrl, token) so repeated constructions skip the redundant `/profile` call (live-verified: 768ms cold, 0ms cached, same real account both times); multi-account selection UX and Inbox/Captain resource-ownership validation remain open, see the audit entry; HAR-003 closed by PR #219 — event delivery no longer trusts `conversations[0]`, a shared `selectConversationForInbox()` requires real `inbox_id` membership and fails closed otherwise; HAR-017 partially closed by PR #221 — an unconfigured optional module (Support API/MCP/verification) no longer drags overall Support Gateway health to degraded; the fuller module-state model (last-verified-healthy vs stale vs failed, timestamps, reason codes) remains open; HAR-012 partially closed by PRs #223/#227/#231 — confirmed all 8 real event types are v2-owned (dispatchEvent()'s v1 event-handler call sites are now provably dead code), removed both dispatchEvent()'s and syncEmailTemplates()'s opportunistic queue drains (ProcessLegacyRetryQueueScheduledTask is now the sole drain path anywhere in the plugin), and relabeled retryQueueEnabled/maxRetryAttempts to say explicitly they only tune the legacy path; migrating Send Test Message/canned-response-sync to explicit v2 operations and retiring the legacy queue settings/scheduled task once no producer remains are the only items left open; HAR-013 partially closed by PR #225 (its overlapping "no unrelated-queue drain" item closed jointly by PR #227) — `syncEmailTemplates()` now hard-denies security/verification templates (keyword-matched) before ever calling `createCannedResponse()`, live-verified against this real installation's actual templates (6 of 112 denied: MAGIC_LOGIN_LINK, PASSWORD_RESET_CONFIRM, REVIEWER_REGISTER, USER_REGISTER, USER_VALIDATE_CONTEXT, USER_VALIDATE_SITE); the larger opt-in-feature-vs-remove-the-button product decision remains open.
