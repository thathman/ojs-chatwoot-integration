...
Last reconciled: 2026-09-03 (PR #207 resolved and live-verified — UX-024 canonical-settings first slice complete).
...
2. **Settings Console Redesign — in progress.** `docs/v2/SETTINGS_UI_REDESIGN.md` is the authoritative product/UX brief. The canonical-settings first slice (UX-024, ADM-007) is now **complete**: `SettingsRegistry`/`SettingDefinition` (`classes/v2/Settings/`, PR #200) is the single source of truth for every one of the 39 real setting keys, and every previously-duplicated key list in the plugin — `ChatwootIntegrationBasePlugin::exportKeys()`, `ChatwootIntegrationV2Plugin::legacyExportKeys()`, `ExportPolicy::sensitiveKeys()`, `guessSettingType()`, `saveGlobalProfile()`/`applyGlobalProfile()`'s global-eligibility check, and `ChatwootSettingsForm`'s `initData()`/`readInputData()`/`execute()`/`secretKeys()` (PRs #201/#204/#205/#207) — now delegates directly to it, with `tests/v2/settings-registry.php` as an automated drift guard. HAR-008 closed as a side effect. Live-verified on dell (real `initData()` invocation against the real AJDSI journal). Next: build the actual tabbed UI (Overview/Chatwoot/Widget/Automation/AI & Knowledge/Verification/API & MCP/Integrations/Advanced) — ADM-008/ADM-009. Console foundation comes before further Knowledge/Staff/Provider/Payment UI expansion.
