...
**Still not exercised this pass** (real further defects may exist here, not yet checked): Chatwoot tab's discovery/inbox/Captain-assistant selectors, Widget tab's structured Appearance controls + local preview + real-frontend-vs-preview match, AI & Knowledge tab, positive-audience controls on the Widget tab, keyboard-only tab navigation, narrow/responsive layout, duplicate-DOM-ID check across the whole rendered page at once. Continue the K checklist from here next.
