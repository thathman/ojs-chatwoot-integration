...
Anti-enumeration is enforced structurally: the endpoint has exactly one success response call site (`{ok:true, data:{verificationRequested:true, challenge:"<32 hex chars>"}}`), and every branch — email not found, account disabled (PKP's own `getByEmail(..., allowDisabled: false)` already makes these indistinguishable), rate-limited, cooldown, or a mail-send exception — falls through to it silently. `challenge` is always populated (TST-019): a real request gets the real challenge's `public_reference`; every other branch gets a same-shape (`bin2hex(random_bytes(16))`) value that simply never matches a stored row, so a real client can always call `verificationConfirm` with the value this endpoint returned, and its presence never reveals whether the claimed email matched an account. Response *content* is proven identical by a source-level test; response *timing* is padded up to a fixed floor (`ResponseTimingNormalizer`, IDN-015) on every branch, closing the common case (the fast "not found" path no longer returns near-instantly next to a slower real send) — but the normalizer only ever adds delay, never removes it, so an unusually slow real mail-send that exceeds the floor remains distinguishable from the padded "not found" path. This is a known, accepted residual gap, not a claim of full timing-side-channel closure.
...
As of the Provider Registry phase (`docs/v2/AIRIX360_TASKLIST.md` AXP-*/APS-*), the response additionally carries an `obligations` array — one entry per registered payment provider that reports something for this submission, each `{producer, feeKey, status, amount, payableAmount, currency, payUrl}`. It is empty unless the Airix Submission Fee sibling plugin (`Airix360/submissionFee-OJS`, verified against `1.7.0.0`) is installed, enabled, and version-compatible. When it does report an obligation, that provider — not the native publication fee — becomes the authoritative `status`/`amount`/`currency` for the top-level fields (a journal configures one real fee producer at a time; see `AIRIX360_INTEGRATIONS.md` §5.8 on producer vs. collector). `status` there additionally covers `waived`/`partially_waived`/`refund_review`/`refunded`, sourced from the provider's own `PaymentHelper` (never re-derived waiver-percent math), and now also `unknown` (`PaymentObligationStatus::UNKNOWN`): when the Airix provider is installed but genuinely fails (throws, or reports a broken health state — as opposed to legitimately not applying, e.g. disabled), the top-level `status` becomes `unknown` rather than silently falling back to the native OJS producer's paid/unpaid state, which could otherwise report a completely unrelated fee's status as if it were the real answer (docs/v2/PAYMENT_PORTFOLIO.md). Gateway/orchestrator adapters (Paystack/Flutterwave/Bachs/MultiPay) and the waiver-plugin's own public knowledge/actions are not built yet.

A top-level `payUrl` field (docs/v2/AIRIX360_TASKLIST.md PTF-011) is `null` unless `status` is genuinely payable (`unpaid`/`partially_waived`) *and* the winning obligation actually returned one — never constructed/guessed here. When present, `availableActions` gains a `pay` descriptor. `retry`/`request_waiver`/`view_history`/`wait`/`contact_support` are not implemented — no verified real destination exists for any of them yet.
