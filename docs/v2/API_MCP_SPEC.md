...
`supportState` now additionally distinguishes `revision_requested` (review-stage submissions whose current review round has status `REVISIONS_REQUESTED`, read live from `ReviewRoundDAO`, verified against `pkp-lib` stable-3_5_0 — never cached or recomputed independently) from plain `review_in_progress`. Still not attempted: `draft` (would need a `submissionProgress`-based candidate path, since a draft has no stage assignment to discover it through) and `revision_received`.
