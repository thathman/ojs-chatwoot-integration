...
| POL-011 | Resource/relationship-aware reviewer masking (replace v1 role-wide masking) | **COMPLETE (unit/structural), POL-015 open (real-browser acceptance)** | Closed via `classes/v2/Relationship/ReviewerMaskingPolicy.php`/`CurrentSubmissionResolver.php`, wired into `addChatwootWidget()`. Masking now derives from a real, resource-scoped OJS relationship (author/editorial/manager/site_admin on the current submission) rather than the journal-wide Reviewer role alone; the role-wide check remains only as the fail-closed default when no resource is resolvable. `tests/v2/pol-011-resource-aware-reviewer-masking.php` proves the exact author-on-A/reviewer-on-B scenario plus fail-closed paths. Real-browser acceptance on live OJS 3.5 is POL-015, not yet done — do not conflate this unit/structural evidence tier with full acceptance. |
...
| ---- | -------------- | -------- |
| `server/discover` handler | **COMPLETE (unit-tested)** | Closed via MCP-010: registered in `mcpRequest()`, reports the real `McpProtocol::REVISION`/`SUPPORTED_METHODS` plus the same real tool/resource lists `tools/list`/`resources/list` expose. `tests/v2/mcp-010-server-discover.php` proves a real dispatcher round trip. Real-client acceptance against the live MCP gateway not yet separately re-run. |
