...
- [x] **PTF-008** Ensure conflicting provider evidence resolves to `attention_required`/`unknown`, never guessed state. — partial (the "genuine provider failure -> unknown" half): `SupportProviderRegistry::resolveObligations()` now returns an `ObligationResolution` distinguishing a provider that legitimately doesn't apply (disabled/not-installed/incompatible-version — silently skipped, not a failure) from one that genuinely failed (threw, or reported `DEGRADED`/`UNAVAILABLE`/`UNKNOWN` health). `ojs_get_payment_status` reports `PaymentObligationStatus::UNKNOWN` on a genuine failure instead of silently falling back to the native OJS producer's paid/unpaid state, which could otherwise report a completely different fee's status as if it were the real answer. Multi-provider *conflicting* evidence (two producers both reporting an obligation) is not yet a real scenario — only one producer (Airix Submission Fee) exists today, so `attention_required` for that case is not implemented.
...
- [ ] **PTF-011** Add safe public financial action descriptors (`pay`, `retry`, `request_waiver`, `view_history`, `wait`, `contact_support`). — partial: only `pay` is implemented — `PaymentStatusSerializer` surfaces a top-level `payUrl` and adds a `pay` action descriptor only when the status is genuinely payable (`unpaid`/`partially_waived`) *and* a provider actually returned a real `payUrl` (never fabricated/constructed here). `retry`/`request_waiver`/`view_history`/`wait`/`contact_support` are not implemented — no verified real URL/action exists for any of them yet (e.g. no verified public "request waiver" submission endpoint), and this codebase does not fabricate action links it can't back with a real destination.
